...
export function report(result) {
  ...
  lines.push(LIMITATION, `Limits: ${LIMITS.pattern} characters per pattern; ${LIMITS.path} per path; ${LIMITS.provided} provided lines; ${LIMITS.braces} brace combinations and ${LIMITS.wildcards} wildcard tokens per pattern; ${LIMITS.timeout / 1000}-second Worker deadline.`, 'Search uses finite literal-derived templates, wildcard lengths, directory depths, extension, dotfile and case variants; it cannot enumerate all paths.', 'Generated examples are synthetic paths, not files known to exist in a project.', 'Examples are sorted by length then code-unit order; these are shorter examples found, not globally shortest witnesses.', 'Picomatch semantics may differ from Git, Docker, Bash, GitHub Actions or other tools.', '', '| Result | Source | Path |', '| --- | --- | --- |');
